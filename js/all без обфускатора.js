...
function getRedirectUrl(targetTableId) {
  ...
  const prefix = isHome ? './' : '../';
  const suffix = isLocal ? 'index.html' : '';
  let relativePath = '';

  switch (targetTableId) {
    case 'table1': relativePath = isHome ? (isLocal ? './index.html' : './') : (isLocal ? '../index.html' : '../');
      break;
    case 'table2': relativePath = prefix + 'table-ppo-2/' + suffix; 
      break;
    case 'table3': relativePath = prefix + 'search-ppo/' + suffix;
      break;
    case 'table4': relativePath = prefix + 'search-ppo-2/' + suffix;
      break;
    case 'table5': relativePath = prefix + 'choice-ppo/' + suffix;
      break;
    default: relativePath = prefix + suffix;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (currentPage === 'table1' && lastSelected !== 'table1') {
    ...
    // Стираем "index.html" из обоих путей перед сравнением (чтобы не было вечных перезагрузок)
    const normCurrent = window.location.href.replace(/index\.html$/, '');
    const normDest = destination.replace(/index\.html$/, '');

    if (normCurrent !== normDest) {
